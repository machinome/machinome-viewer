...
- [ ] 0.1 Record the baseline this change starts from, in this file: the
...
- [ ] 0.2 Copy the framework's committed corpus into the widget source,
...
- [ ] 0.3 Copy the acceptance document into
...
- [ ] 1.1 Red: `src/run/running-corpus.test.ts` — the whole conformance
...
- [ ] 1.2 Red: in the same file, the width guard — a mirror of the
...
- [ ] 2.1 Red: `src/run/scope.test.ts` — `nest({'units.drum.turn': 7})` is
...
- [ ] 3.1 Red: `src/run/program.test.ts` over hand-written programs — every
...
- [ ] 3.2 Red: the same file — what loading a **good** program yields: the
...
- [ ] 3.3 Green: `src/run/program.ts`.
- [ ] 3.4 Red then green: the held node ids are generation-guarded (design
...
- [ ] 4.1 Red: `src/run/edges.test.ts` — `values`, `increments`, `cuts`,
...
- [ ] 4.2 Green: `src/run/edges.ts`.
- [ ] 4.3 Red then green: every evaluation gets a FRESH scope object
...
- [ ] 5.1 Red: `src/run/jumps.test.ts` — `branchOf` for each of the ten
...
- [ ] 5.2 Red: the partition and the increment — postorder over the jump
...
- [ ] 5.3 Green: `src/run/jumps.ts`.

## 6. Commands

- [ ] 6.1 Red: `src/run/commands.test.ts` — `admits` as a pure function of
...
- [ ] 6.2 Red: the refusals — exactly one of `by`/`to`; a duration that is
...
- [ ] 6.3 Green: `src/run/commands.ts`, reusing `toNative` from
      `drivers.ts` for the design→native conversion.

## 7. The run

- [ ] 7.1 Red: `src/run/run.test.ts` — one pass over the edges in program
...
- [ ] 7.2 Red: the stop machinery — `reached` (outside a bound AND further
...
- [ ] 7.3 Red: the segment loop and atomicity — a tick with two stops at
      ...
      retired `refused`.
- [ ] 7.4 Red: state — `snapshot` carrying identity, `dt`, tick, bank and
...
- [ ] 7.5 Green: `src/run/run.ts`.

## 8. The engine, and the corpus turning green

- [ ] 8.1 Green: `src/run/engine.ts` — the façade tasks 1.1 and 1.2 were
      ...
      `Sim._ticks`.
- [ ] 8.2 Run the corpus suite. Every one of the thirteen scenarios, all
...
- [ ] 8.3 Record the cost: ticks per second for `CarryLead` (affine plans,
      solved) and for a searched plan, in node, so a later regression is a
      number rather than an impression.

## 9. The document admits version 5

- [ ] 9.1 Red: `src/document.test.ts` — `RENDERED_VERSIONS` is
...
- [ ] 9.2 Red: the same file — every existing version 1 to 4 case passes
...
- [ ] 10.3 Green: `src/run/protocol.ts`, `src/run/worker.ts`,
      `src/run/runtime.ts`.
- [ ] 10.4 Green: `build.mjs` — the first pass bundling `src/run/worker.ts`
...
- [ ] 11.1 Red: `src/run/pose.test.ts` — the scope a committed bank makes
...
- [ ] 11.2 Green: `viewer.ts` — a version 5 document starts a runtime,
...
- [ ] 11.3 Green: the `run()` handle of design §7, with `runsInWorker`, and
      `run()` returning `null` for a document with no program.
- [ ] 11.4 Red then green: `src/options.test.ts` — the
...
- [ ] 11.5 Red then green: a mounted run is stopped (design D13) — mounting
...
- [ ] 12.1 Red then green: `package.json` — `solidNodeViewerApi` 7 → 8 and
...
- [ ] 12.2 Red then green: `tests/test_bundle.py` and `tests/test_cli.py` —
...
- [ ] 12.3 Green: `tests/test_widget_e2e.py` — the mounted handle and the
      global report API version 8.

## 13. The acceptance: the Pascaline module in a real browser

- [ ] 13.1 Red: `tests/test_running_document.py`, a Playwright test on the
...
- [ ] 13.2 A screenshot of the fixture at rest and after the ten
      ...
      not an inspection.
- [ ] 13.3 Record whether the run used the worker (`runsInWorker`) in that
      page, and the wall time the ten instructions took.

## 14. The whole suite, the docs and the records

- [ ] 14.1 `npm run typecheck` and `npm test` in `solid_node_viewer/widget`
