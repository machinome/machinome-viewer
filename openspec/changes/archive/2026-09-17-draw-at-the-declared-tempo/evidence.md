...
Integration note (2026-09-22): this evidence describes the original
pre-rename implementation. The accepted decision was renumbered ADR-073
because the Machinome rename had already assigned ADR-066. Current viewer
API and document-version declarations are tracked by later decisions.
The integration into the renamed 0.7 viewer passed TypeScript typecheck,
bundle build, `openspec validate --all --strict` (10 items), and the real
browser `GestureDrawnInABrowserTest` (1 passed). The widget suite passed
1454 of 1456 tests on its first run while another browser/pytest workload
ran concurrently; the two failed throughput thresholds in `run/cost.test.ts`
both passed when that file was rerun alone (14 of 14). No behavior assertion
failed. The current manual now describes the declared-tempo rule.
