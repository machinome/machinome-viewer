...
- [x] 0.1 Confirm `run-in-the-worker` is implemented and integrated in this
      repository, and record its head here. This change has no run to
      drive without it; do not start on a branch that lacks it.
- [x] 0.2 Record the baseline: `npm test`, `npm run typecheck`,
...
- [x] 1.1 Red: `src/runControls.test.ts` in plain node — `runControlLayer`
...
- [x] 2.2 Green: the choice — a document carrying a program builds the
...
- [x] 2.3 Green: the jog's five release paths — `pointerup`,
...
- [x] 2.4 Green: a request issued while the run is paused starts it.
- [x] 2.5 Green: outcomes — each control shows its last outcome
...
- [x] 2.6 Red then green: `src/options.test.ts` — the `run: {nudge: {amount,
...
- [x] 3.1 Red: `src/run/runtime.test.ts` (or a new `republish.test.ts`) —
      ...
      second path.
- [x] 3.2 Green: `viewer.ts`'s `manifestChanged`, and the panel line saying
      which of the two happened.

## 4. The capture

- [x] 4.1 Red: `tests/test_capture.py` and `tests/test_cli.py` — a staged
...
- [x] 4.2 Green: `capture.py` and `cli.py`. Nothing here talks to the
      framework: it learns what this package supports from
      `documentVersions`.

## 5. The acceptance: a maker drives the Pascaline

- [x] 5.1 Red: `tests/test_running_document.py` gains the on-screen half,
