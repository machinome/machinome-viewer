...
- [x] 0.1 Confirm the base: this worktree is `execute-the-self-read` off
...
- [x] 0.2 Record the baseline in
      ...
      - the four cost measurements `src/run/cost.test.ts` prints on this
        ...
        32 942 ticks/s.
- [x] 0.3 Record the CORPUS GAP on the base, before changing anything.
...
- [x] 0.4 Read, and keep open, the framework's own record at
      ...
      `program.py` is right.
- [x] 0.5 Confirm the refusal this cycle removes is real, and paste it:
...
- [x] 1.2 Update the census in `src/run/running-corpus.test.ts`: 17
      machines entries, 14 distinct names, 328 ticks.
- [x] 1.3 Red: `npm test -- running-corpus` fails on `Clearing` (both
      step sizes) and `StoppedClearing` and on nothing else. Paste it
      verbatim; it is 0.3's list minus the census assertion.
- [x] 1.4 Widen the width guard's `REQUIRED` to the producer's list
...
- [x] 2.1 Red, in `src/run/program.test.ts`: a hand-written version 6
...
- [x] 2.2 Add `retained: (RetainedReading | null)[]` to `ProgramEdge`,
...
- [x] 2.3 Dependence: a jump depends on the driven id when that id is
...
- [x] 2.4 The three load-time refusals (design D1), each red first, each
      ...
      hold the driven id.
- [x] 2.5 Tests for what is NOT refused: a version 6 document whose
...
- [x] 3.1 Red, then write `ordinalOf`, `fromOrdinal`, `ulpOf`,
...
- [x] 3.2 Tests against values the producer's own functions answer:
...
- [x] 4.1 `retainedIncrement(program, reading, start, delta, described,
...
- [x] 4.2 The walk's constructor zeroes the driven coordinate's own
...
- [x] 4.6 `firstCut`: the solved path where `jump.affine &&
...
- [x] 4.7 Tests for 4.6 written as the three defects the framework's
...
- [x] 4.8 `land`/`farSide`: nothing to walk where the piece did not move
...
- [x] 4.9 `LandingInvariantError` where no bracket is found within 200
...
- [x] 4.10 `maxCrossings` refuses the walk with the existing `tooMany`
      message and count.

## 5. Reporting and committing (`src/run/edges.ts`, `src/run/run.ts`, design D4, D5)

- [x] 5.1 `edgeIncrements` gains a `landings: Record<string, number> |
...
- [x] 5.2 `Run.pass` threads `landings`; `Run.integrate` applies them to
...
- [x] 5.3 `RefusalKind` gains `'landing'`; `refusalKind` maps the new
      ...
      after a cut.
- [x] 5.4 A self-read crossing is recorded as a crossing and never as a
...
- [x] 6.1 The `Clearing` bench, taken from the corpus's own copy of the
      ...
      `STATION = (100, 500)`.
- [x] 6.2 The dial clears to its gap and the ring runs on: a sweep long
...
- [x] 6.3 A second and a third sweep move the dial by NOTHING — the same
      float, bit for bit (`Object.is`), not within a tolerance — and
      every command completes.
- [x] 6.4 Swept backward from inside the station, the dial ends on the
      band's UPPER edge and does not move on a further backward sweep.
- [x] 6.5 A dial standing exactly at a band edge holds in the direction
      that would take it deeper and turns in the direction that leaves —
      rule (c) from the outside.
- [x] 6.6 The same sweep at one tick, twelve and two hundred and forty
      agrees within the run's agreement window, with no stop and no
      blocked command.
- [x] 6.7 A run state taken after a partial sweep and restored resumes
      from the same float and reaches the same band edge.
- [x] 6.8 The `StoppedClearing` bench: one segment reports both a landing
...
- [x] 8.2 Red, then `solidNodeViewerApi` `14 → 15`, with the comment in
      ...
      stays skipped.
- [x] 8.3 `viewer.ts`'s program gate `document.version === 5 ||
...
- [x] 9.1 Produce `tests/fixtures/clearing/viewer.json` from a
      **throwaway copy** of a solid-node checkout at main `8e15791`
      ...
      non-empty.
- [x] 9.2 Stand the geometry in: each distinct model path the document
...
- [x] 9.3 A vitest case loading the fixture through `Engine.load`: it
...
- [x] 9.4 The browser acceptance, beside `tests/test_lock_document.py`'s
...
- [x] 9.5 Two inspected screenshots written to `tests/_shots/`, as the
...
- [x] 10.1 Extend `src/run/cost.test.ts` with two printed measurements,
...
- [x] 10.2 Assert the other way too: `Train`'s existing measurement does
      not move. A document with no self-read edge pays one array-length
      test per edge per tick and nothing else.
- [x] 10.3 Record both in evidence beside the framework's own numbers
...
- [x] 11.1 `npm run typecheck` clean, `npm test` green with its new
      count, `npm run build` writes `dist/solid-widget.js`. Paste all
      three.
- [x] 11.2 The Python suite green with the rebuilt bundle, naming every
      skip. Paste it.
- [x] 11.3 `acceptance.test.ts` — the Pascaline module — is unchanged to
      the digit: `tens.drum.turn` still `65.54` after ten `Add one`. A
      document with no self-read edge does not move.
- [x] 11.4 `test_lock_document.py` unchanged: a version 5 document with
      constraints runs exactly as it did.

## 12. The record

- [x] 12.1 `CHANGELOG.md`, a new bullet under `0.2.0 — unreleased`: the
...
- [x] 12.2 `docs/adrs/EXPORT/ADR-057-a-self-read-edge-is-executed-from-the-retained-value.md`,
...
- [x] 12.3 `openspec validate execute-the-self-read --strict` clean, the
      ...
      evidence.
- [x] 12.4 Report to the pilot, as findings rather than fixes: whether
