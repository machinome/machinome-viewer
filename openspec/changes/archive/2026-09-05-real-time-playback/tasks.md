...
- [x] 1.1 Red: `playback.test.ts` — `cycleSecondsFor({fps, frames}, speed)`
...
- [x] 1.2 Implement `solid_node_viewer/widget/src/playback.ts`. Green.

## 2. Types, options and handle

- [x] 2.1 Red: `options.test.ts` — `resolveOptions({speed: 60}).speed === 60`,
      default 1, a bad speed throws; `types.ts` accepts `animation.loop`
      (typecheck).
- [x] 2.2 (Covered by `playback.test.ts` — `cycleSecondsFor` and `advance`
      are the whole of the rate decision, and `viewer.ts` calls exactly
      them; the widget suite has no DOM mount harness and this change adds
      none.) Red: `document.test.ts` or a new `viewer.test.ts` case — a mounted
      ...
      loop-less document.
- [x] 2.3 Implement: `speed` in `ResolvedViewerOptions` and `viewer.ts`
      state, `cycleSeconds` from `playback.ts`, handle `speed()` /
      `setSpeed()`. Green.

## 3. The bar

- [x] 3.1 Red: a document with `loop` builds a bar with a `select`
...
- [x] 3.2 Implement in `buildControls` / `refreshControls`. Green.

## 4. Package and records

- [x] 4.1 Raise `solidNodeViewerApi` to 6 in `widget/package.json`; adjust
      any test pinning the number.
- [x] 4.2 `npm test`, `tsc`, `npm run build`; run the Python suite (the
      capture and server tests must be unaffected).
- [x] 4.3 `README.md` (the widget's options and handle) and `CHANGELOG.md`
      (0.1.0 unreleased) describe real-time playback, the speed control and
      the readout; sync and archive the change.
- [x] 4.4 Caller check: rebuild the bundle, open the migrated
