...
- [x] 1.1 Copy the framework's `tests/running-corpus.json` at `0b0f02a`
      over `solid_node_viewer/widget/src/running-corpus.json`, BYTE FOR
      BYTE. Nothing here edits it, ever.
- [x] 1.2 Update the census in `run/running-corpus.test.ts`: 19
...
- [x] 1.3 Record the replay suite's state with the new file and the old
...
- [x] 1.4 Add the producer's three features to `REQUIRED`, in the
...
- [x] 1.5 Implement their detection in `uncoveredFeatures`, mirroring
...
- [x] 1.6 Extend the narrowed-corpus test to name one of the three, as it
      already names the self-read's three.

## 2. The walk's own arithmetic (design D5), red first

- [x] 2.1 A red test on a VERSION 6 document with no block anywhere: the
      ...
      `71.99999999999994`.
- [x] 2.2 Parenthesise `Walk.run`'s `ownAt` closure
      (`jumps.ts:566-567`) and `Walk.probe` (`jumps.ts:700`) as
      `ownLeft + (S − base)`. Nothing else in the walk changes.
- [x] 2.3 Green, and the seventeen pre-existing corpus scenarios still
...
- [x] 3.1 `ProgramBlock` and `EdgeKind` gain `'block'` in `program.ts`.
...
- [x] 3.2 The dependency graph and iterative Tarjan over it, in the
...
- [x] 3.3 Contraction in place at the first member's index —
...
- [x] 3.4 The order VERIFIED, not re-sorted (design D1.2): every entry's
...
- [x] 4.3 MONOTONICITY pinned, as the producer pinned it (task 2.4
      there): the reads under the all-zero fold are a SUBSET of the reads
      under each single zero.
- [x] 4.4 `selectorsOf(plan, blockGives)` — `_selectors`
...
- [x] 4.5 `unconditional` and `switched` per member, own give excluded
...
- [x] 5.1 A wiring or a formula on a cycle. Red first, message naming the
      edge, its `statedBy` and every member.
- [x] 5.2 A member driving a GROUP (more than one give). Red first.
- [x] 5.3 A member whose give is not a bank coordinate. Red first.
- [x] 5.4 A cycle no selection can break — `unconditionalCycle`
...
- [x] 5.5 Each of the four is refused at LOAD, quoting the document, with
      nothing rendered and no tick taken — the surface `program.ts`
      already stands on.

## 6. The block's step (design D2)

- [x] 6.1 `blockIncrements` reached from `edgeIncrements` on
      `kind === 'block'`; `edgeCuts` and `edgeValues` likewise.
- [x] 6.2 The selector partition over the whole stretch
...
- [x] 6.3 The midpoint branches per piece (`_Block._forced`), and the
...
- [x] 6.4 A still-cyclic piece refuses the tick with `_Block._refused`'s
...
- [x] 6.5 The members run over the piece: sources the block does not give
...
- [x] 6.6 **The in-block value ADVANCED between pieces, with the
...
- [x] 6.7 Crossings rescaled `left + t·(right − left)` per piece, the
      ...
      correctly.
- [x] 6.8 The two-argument call shape (design D2.7): complete,
...
- [x] 7.1 `forced` added to `partition` and `branchesAt` — the ONLY two
      ...
      the level.
- [x] 7.2 `planIncrement`, `planCuts`, `retainedIncrement`,
...
- [x] 7.3 Test that a member whose selector sits in layer one and whose
      ...
      `0.0 != 1.0`).
- [x] 7.4 Test that a forced node's crossings are NOT located inside the
      piece (the producer's other reverted measurement:
      `[Crossing(..., primitive='>=', level=0.0, t=0.5)] != []`).
- [x] 7.5 A law with no block pays exactly one extra optional argument
      and nothing else: the seventeen pre-existing corpus scenarios stay
      byte-identical.

## 8. What a block reports (design D4)

- [x] 8.1 The ADVANCED ABSOLUTE at the stretch's end for every coordinate
...
- [x] 8.5 `run.ts` has no structural change. State it in the commit and
...
- [x] 9.1 The one-tick table of task 0.4, turned into an assertion: the
...
- [x] 9.2 The producer's frozen-twin table at `dt = 1/12` over 12 ticks:
...
- [x] 9.3 A selection change ALONE moves nothing — exactly the number
      zero, bit for bit, not approximately.

## 10. The corpus, green

- [x] 10.1 All 19 scenarios replay green, exactly for discrete state and
      within the corpus's own `1e-9` relative window for floats.
- [x] 10.2 The width guard green on the new file, red on the old and on a
      trimmed one.

## 11. Versions (design D7)

- [x] 11.1 Red first: `version.test.ts` for `documentVersions`
      `[1, 2, 3, 4, 5, 6, 7]` and `RENDERED_VERSIONS` following it in
      `viewer.ts`.
- [x] 11.2 Red, then `solidNodeViewerApi` `15 → 16`, with the comment in
      `package.json` and the spec's own sentence agreeing.
- [x] 11.3 The spec scenario "A version beyond the ones it reads is
      refused" now mounts a version 8 document; the Python and widget
      tests that pin the refused version follow.
- [x] 11.4 `bundle.py`'s `RELEASED_DOCUMENT_VERSIONS` does NOT move;
      assert `describe()['documentVersions']` is the package's new list.
- [x] 11.5 `capture.py`'s `carries_program` and `viewer.ts`'s program
...
- [x] 12.1 Export `tests/carriage_project/machine.py:CurtaCarriage` from
      ...
      `identity`.
- [x] 12.2 Commit it as `tests/fixtures/carriage/` with a README stating
...
- [x] 12.3 A document test with no browser: version 7, five drivers,
...
- [x] 12.4 A Playwright acceptance beside `tests/test_clearing_document.py`,
...
- [x] 13.2 The ratios recorded against the producer's (1.6x a frozen
...
- [x] 14.1 `npm test`, `npm run typecheck`, `npm run build` — against the
      0.1 baseline, with the new totals recorded.
- [x] 14.2 The Python suite from the worktree root with the workspace
      venv, and `scripts/check-dist` (which uploads nothing).
- [x] 14.3 The rebuilt `dist/solid-widget.js` committed, as the previous
      cycles committed theirs. **NOT DONE, and not doable as stated:**
      `/solid_node_viewer/widget/dist` is in this repository's
      `.gitignore` and `git log -- solid_node_viewer/widget/dist` is
      EMPTY — no cycle has ever committed the bundle here. The bundle IS
      rebuilt (723.9 kb) and every Python suite ran against it; there is
      nothing to stage. See evidence.md.

## 15. The record
  (Reviewer: closed as NOT APPLICABLE as stated — `solid_node_viewer/widget/dist` is gitignored and no cycle has ever committed it; the bundle was rebuilt and every Python suite ran against it, and the primary checkout's bundle is rebuilt at integration, as the previous cycles did.)
- [x] 15.1 `CHANGELOG.md` under `0.2.0 — unreleased`: the block, the
      per-piece order, the version 7 document, the walk's ulp, and the
      carriage in a browser.
- [x] 15.2 `README.md`'s version claims.
- [x] 15.3 `docs/adrs/EXPORT/ADR-058-a-block-is-ordered-per-piece-from-the-published-edges.md`,
      extracted AFTER implementation, and `docs/adrs/README.md` updated.
      **LEFT FOR THE REVIEWER** (apply briefing item 6).
- [x] 15.4 `evidence.md` for this change: the measurements of §0 with the
      after numbers beside them, the deviations from this design and why,
      and the follow-ups.
- [x] 15.5 `openspec validate execute-the-selection --strict` green (it
      is: "Change 'execute-the-selection' is valid"), and the change
      archived under its dated name after implementation. **The ARCHIVE
      is left for the reviewer** (apply briefing item 6).
