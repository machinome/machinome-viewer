...
- [x] 1.1 In `src/clocked/clocked-corpus.test.ts`, move the census to the
...
- [x] 1.2 Copy the producer's `tests/clocked-corpus.json` over
      ...
      cycle's corpus red.
- [x] 1.3 Add to `replayStep` the comparison of BOTH ENDS on every step,
...
- [x] 2.1 `src/clocked/machine.test.ts`: a `by` request from a non-zero
...
- [x] 2.2 `ClockedRequest` gains `origin` and `end`, returned from the
...
- [x] 3.1 `machine.test.ts`, red: `trigger('Stroke')` over the corpus's
...
- [x] 3.2 Implement `trigger` on `clockedMachine`: resolve the
...
- [x] 3.3 The corpus replay's `trigger` verb, in `replayStep`, recorded
...
- [x] 4.1 `src/clocked/document.test.ts`, red: a version 8 document
...
- [x] 4.2 Implement it in `readInstructions`, in the refusal shape the
...
- [x] 5.1 `src/clocked/drawing.test.ts`, red, over hand-written requests
...
- [x] 5.2 Write `src/clocked/drawing.ts` — the request, the start bank,
...
- [x] 6.1 `viewer.ts`: hold at most one drawing; `clockedTrigger(name)`
...
- [x] 6.2 Landing: every other path — `clockedRequest`, the handle's
      `move`/`restore`/`reset` wrappers, `clockFrame`, another trigger —
      lands the drawing first. Starting a drawing pauses the clock's
      transport.
- [x] 6.3 The chrome: `ClockedInstructionControl` carries an outcome
...
- [x] 6.4 The posed `ViewerHandle.trigger` refuses by name under a
      clocked document, pointing at `machine().trigger`.
- [x] 6.5 `npx tsc --noEmit` and `npx vitest run`: green, with the base's
      1168 tests still green and the new ones beside them.

## 7. The acceptance, in a real browser

- [x] 7.1 Re-export `tests/fixtures/calculator/` from a THROWAWAY COPY of
...
- [x] 7.2 The drawing acceptance (design §20, items 20-25): the per-frame
...
- [x] 7.3 `tests/test_curta_drawing.py` (new, skipped where the build is
...
- [x] 7.4 `pytest tests -q` from the worktree root: green, with every
      skip printing its reason.

## 8. Measure, and say it in numbers

- [x] 8.1 The Curta's drawing: frames per second, median and worst frame,
      against `fast_curta`'s posed ramp, same session. Recorded in
      `evidence.md` whatever it says.
- [x] 8.2 `trigger` against the same `move` on the corpus `Calculator`,
      median of 20, in thread.
- [x] 8.3 The bundle's size before and after, and the widget suite's wall
      time before and after.

## 9. The record

- [x] 9.1 `npm run build` in THIS worktree's widget, so
...
- [x] 9.2 `package.json`: `solidNodeViewerApi: 19`,
      `solidNodeDocumentVersions` unchanged; `src/version.test.ts` and
      `tests/test_version.py` green.
- [x] 9.3 `docs/adrs/EXPORT/ADR-064`, extracted after implementation
...
- [x] 9.4 `CHANGELOG.md` under `0.2.0 — unreleased`, and `README.md`'s
      version table.
- [x] 9.5 `evidence.md`: the red log for every task, the measurements,
      the corpus diff, and anything reported rather than silently
      resolved.
- [x] 9.6 `openspec validate play-the-instruction --strict`, then sync
