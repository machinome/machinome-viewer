...
- [ ] 1.1 `tests/test_documentation.py`: the manifests, the banner template,
      every tracked SPDX header and the reader-facing pages grant
      `AGPL-3.0-or-later`; run it red.

## 2. Relicense

- [ ] 2.1 Replace `AGPL-3.0-only` with `AGPL-3.0-or-later` in every SPDX
...
- [ ] 2.2 Build the bundle from the changed sources and read its banner.

## 3. Record

- [ ] 3.1 Tests green; validate, sync the delta, archive, commit.
