...
- [ ] 1.1 Copy solid-node `tests/clocked-corpus.json` byte for byte to
...
- [ ] 1.2 Write `src/clocked/clocked-corpus.test.ts` against the module
      ...
      the failure count.
- [ ] 1.3 Write the census guard so it derives the departure FROM THE
...
- [ ] 2.1 Lift `Walk.farSide` (`src/run/jumps.ts:1217-1273`) to an
...
- [ ] 2.2 **The running corpus replays byte-identically afterwards.** Run
...
- [ ] 2.3 Unit-test `farSideOf` with a non-zero `scale` at a landed value
...
- [ ] 3.1 Red: `src/clocked/document.test.ts` over the corpus's own
...
- [ ] 3.2 `src/clocked/document.ts`: `loadClocked(document, sourceUrl,
...
- [ ] 3.3 Red then green, one refusal per case, each naming the offending
...
- [ ] 3.4 `assertRenderable` (`src/viewer.ts:2052`, `:2079`) gains the
...
- [ ] 4.1 Red: `src/clocked/events.test.ts` — the level's crossings on a
...
- [ ] 4.2 Green: `src/clocked/events.ts`.
- [ ] 4.3 Red then green: `src/clocked/commit.ts` — the published law
...
- [ ] 4.4 Red then green: `src/clocked/machine.ts` — the request loop
...
- [ ] 4.5 Red then green: `snapshot`, `restore` with the `identity`
      guard, and `reset` (design §12).
- [ ] 4.6 Red then green: `halfToEven` pinned by hand-computed cases
...
- [ ] 4.7 Red then green: the refusal kinds and their mapping onto the
      corpus's `TooManyEvents` / `ClockedError` / `ValueError` /
      `JointRangeError` (design §11).

## 5. The 17 UNBOUNDED machines replay exactly

- [ ] 5.1 Turn the unbounded half of task 1.2 green: the 17 machines that
...
- [ ] 5.2 Where any operation cannot agree bit for bit, **STOP and
...
- [ ] 6.1 Red: with the event solve green, the 13 bounded machines fail
...
- [ ] 6.2 Red then green: `src/clocked/bounds.ts` — per constraint, the
...
- [ ] 6.3 Red then green: the clip ACROSS constraints and the stop report
      ...
      stops.
- [ ] 6.4 Red then green: the END-OF-REQUEST judgement
...
- [ ] 6.5 Green: the 13 bounded machines carry 36 steps, of which **34**
...
- [ ] 6.6 Turn the whole of task 1.2 green: 68 steps, 652 recorded
      numbers, 3 stated clock departures, 5 named deferrals, census
      green.
- [ ] 6.7 Prove the corpus is exact and not merely green: move ONE
...
- [ ] 7.1 Red then green: the clocked pose scope — the bank as the driver
...
- [ ] 7.2 Red then green: `machine(): MachineHandle | null` on the mount
...
- [ ] 7.3 Red then green: `src/clockedControls.ts` — drivers as handles
...
- [ ] 7.4 Red then green, both halves of §13's claim: splitting a
...
- [ ] 7.5 `viewer.ts` renders the clocked chrome from that data, and the
      `$t` timeline is presented exactly as it is for a document carrying
      no machine.

## 8. The capture's animation rule

- [ ] 8.1 Red: a staged version 8 document with a non-zero `--time` is
      ...
      answer.
- [ ] 8.2 Green: split the question — "carries a compiled program"
...
- [ ] 9.1 Export `tests/clocked_project/calculator.py:Calculator`
      ...
      document is edited.
- [ ] 9.2 A fixture test: every model path resolves beside the document,
...
- [ ] 9.3 Playwright acceptance beside `tests/test_clearing_document.py`,
...
- [ ] 10.1 `src/clocked/cost.test.ts` on `src/run/cost.test.ts`'s
...
- [ ] 10.2 Time the same three `Calculator` requests IN THE PAGE, inside
...
- [ ] 10.3 Compare against the running Curta's recorded
