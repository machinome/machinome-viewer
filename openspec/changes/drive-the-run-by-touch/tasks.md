...
- [ ] 0.1 Confirm the base: this worktree is `drive-the-run-by-touch` off
...
- [ ] 0.2 Record the baseline, measured on this base and pasted into
...
- [ ] 0.3 Read, and keep open: solid-node's archived
...
- [ ] 1.1 Commit `tests/fixtures/touched/viewer.json` — the framework's own
...
- [ ] 1.2 Commit the fifteen stand-in meshes it names, each a copy of the
...
- [ ] 1.3 Add `TOUCHED = FIXTURES / 'touched'` and a staging helper beside
...
- [ ] 2.1 Red: `src/partControls.test.ts` in plain node — `readControls`
...
- [ ] 3.1 Red: `src/partControls.test.ts` — the geometry, as pure vector
      ...
      deltas.
- [ ] 3.2 Red: the mode choice — in-plane when
...
- [ ] 3.3 Red: the planner — `quantum = |amount × per_unit|`; a quantum is
...
- [ ] 4.3 Green: the pick — cast against the whole tree, keep the nearest
      VISIBLE hit, act only when that mesh is in the map. A part in front
      of a control is not pressed through.
- [ ] 4.4 Green: the hover affordance — pointer cursor on the canvas, the
...
- [ ] 5.1 Green: the gesture's entry — a capture-phase `pointerdown` on the
      ...
      opens the gesture.
- [ ] 5.2 Green: press versus drag — a press until the pointer has moved
...
- [ ] 5.3 Green: `request(key, unit, issue, at?)` gains the optional client
...
- [ ] 5.4 Green: the drag issues the planner's requests through the same
      `request(...)`, keyed by the input id, so the panel's input row
      reports what the gesture got.
- [ ] 5.5 Red then green: the five release paths — `pointerup`,
...
- [ ] 5.6 Red then green: DOM tests in jsdom for everything jsdom can
...
- [ ] 6.1 Red then green: `src/options.test.ts` — `partControls` defaults
...
- [ ] 6.2 Red then green: `controls()` on the handle — the declaration
...
- [ ] 7.1 Red: a new Playwright class in `tests/test_running_document.py`
...
- [ ] 7.2 Red: **a press.** Move the real mouse over the units dial's
...
- [ ] 7.3 Red: **a blocked drag.** Drag the units dial in the direction the
...
- [ ] 7.4 Red: **a forward drag.** Drag one quantum the other way and
      assert one digit admitted, one request issued, and the drum turned.
- [ ] 7.5 Red: a part the table does not name — press and drag the lid and
      assert the camera moved and no request reached the run.
- [ ] 7.6 Screenshots: the hover affordance, mid-press, after the forward
...
- [ ] 8.1 `npm run typecheck`, `npm test`, `npm run build` and the Python
...
- [ ] 8.2 `README.md`: the opening list says a maker can press and turn the
...
- [ ] 8.3 `CHANGELOG.md`: the same unreleased `0.2.0` section gains the
...
- [ ] 8.4 Write the ADR design §"The ADR to extract" names — the next
      number after ADR-052, in `docs/adrs/EXPORT/` — and add it to
      `docs/adrs/README.md`'s index in order, with its status.
- [ ] 8.5 Record, in `evidence.md`, what the implementation learned about
      each of design §Open Questions and §Reviewer decisions.
- [ ] 8.6 `openspec validate drive-the-run-by-touch --strict` passes.
