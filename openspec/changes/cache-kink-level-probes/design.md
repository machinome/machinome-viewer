...
4. Opt in at the expression-backed running-law and Follow cut paths only where the callback's `program`, kink graph and numeric scope are known. The pinned Curta's 768 Follow boundary cut searches account for 129,622 repeats; 225 law-motion searches account for 24,600. Keep unknown custom binding/value cases on the original path. Existing pure-expression prefix-cache eligibility is related but not a substitute: kink callbacks have different operand roots and lifetime.
