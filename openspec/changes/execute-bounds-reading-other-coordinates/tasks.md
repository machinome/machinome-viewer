...
- [ ] 0.1 Confirm the base: this worktree is `bounds-read-other-coordinates`
...
- [ ] 0.2 Record the baseline in
      ...
      checkout's.
- [ ] 0.3 Read, and keep open, the framework's own record in
...
- [ ] 0.4 Confirm the refusal this cycle removes is real, and paste it
      into evidence: load
...
- [ ] 1.2 Red: `npm test -- running-corpus` fails on `Captured` and on
...
- [ ] 1.3 Widen the width guard in `src/run/running-corpus.test.ts` to
...
- [ ] 2.1 Red, in `src/run/program.test.ts`: a hand-written version 5
      ...
      2.3 land.
- [ ] 2.2 Widen the span free-name check: allowed becomes the bounded
...
- [ ] 2.3 Derive the constraint table in `loadProgram` and publish it on
...
- [ ] 2.4 Tests for 2.3: a bound whose reads arrive only through
...
- [ ] 3.1 `boundsNow()` returns `number | null | Constraint` per side —
      ...
      `locate` for it.
- [ ] 3.2 Red, then write `constraintLevel` (`_constraint_level`,
...
- [ ] 3.3 `constraintBound` (`_constraint_bound`) — the bound evaluated
...
- [ ] 3.4 `constraintReached` (`_constraint_reached`) and
      ...
      `0`.
- [ ] 3.5 `reachedBounds` reproduces `_reached` (`run.py:674`) branch
...
- [ ] 3.6 `constraintGroup` (`_constraint_group`): over the
      ...
      segment's.
- [ ] 3.7 The commit path in `step`: for a constraint entry, NO snap;
...
- [ ] 4.1 The `Gate` bench: inputs `feed` (rest `10`) and `twist` (rest
      ...
      0.05))"}`. `dt = 0.1`.
- [ ] 4.2 The plug blocked while a pin crosses (the STATIC-READS path,
...
- [ ] 4.3 The plug turns once every pin clears: `feed` to `20` over
      `0.4` completes with no stop; then `twist` by `30` completes with
      `30` admitted, `plug.turn == 30` to 1e-9, and no stop.
- [ ] 4.4 Insertion and turning in one tick: `feed` by `10` and `twist`
...
- [ ] 4.5 Withdrawing from a turned plug (the SAMPLED path, and a stop
      ...
      holds.
- [ ] 4.6 The same travel at any cadence: the same withdrawal in 1, 4
      and 40 ticks agrees on the final travel and the admitted travel to
      1e-9, all three `blocked`.
- [ ] 4.7 A constraint stop replays identically from a snapshot taken
      before it: same travel, same admitted, same stop record.
- [ ] 4.8 The `Captured` bench — `Gate` resting seated (`feed` rest
...
- [ ] 4.9 Returning the plug and withdrawing in one tick: `twist` by
...
- [ ] 4.10 The `PawlRatchet` bench — inputs `arbor` (rest `40`) and
...
- [ ] 4.11 A quiet bound costs nothing: a tick moving nothing a
...
- [ ] 5.1 `npm test -- running-corpus` passes: all 14 scenarios, 276
      ticks, exactly for discrete state and within `1e-9` relative for
      floats. Paste the count.
- [ ] 5.2 The width guard passes on the new corpus and fails on a
      narrowed copy naming one of the two new features.

## 6. The whole suite, and the build

- [ ] 6.1 `npm run typecheck` clean, `npm test` green with its new
      count, `npm run build` writes `dist/solid-widget.js`. Paste all
      three.
- [ ] 6.2 The Python suite green with the rebuilt bundle, naming every
      skip. Paste it.
- [ ] 6.3 `acceptance.test.ts` — the Pascaline module — is unchanged to
...
- [ ] 7.1 Commit `tests/fixtures/lock/viewer.json`:
...
- [ ] 7.2 Commit the fifteen stand-in meshes the document names, each a
...
- [ ] 7.3 A vitest case loading the lock fixture through `Engine.load`:
...
- [ ] 7.4 The browser acceptance, beside
...
- [ ] 7.5 Two inspected screenshots written to `tests/_shots/`, as the
...
- [ ] 8.1 Extend `src/run/cost.test.ts` with three printed measurements,
...
- [ ] 8.2 Assert the other way too: `Train`'s existing measurement does
      not move. A machine declaring no such bound pays nothing.
- [ ] 8.3 Record all three in evidence beside the framework's own
...
- [ ] 9.1 `CHANGELOG.md`, a new bullet under `0.2.0 — unreleased`: the
...
- [ ] 9.2 `README.md` only where a statement actually changes. Check the
      run and conformance paragraphs and change nothing else.
- [ ] 9.3 `docs/adrs/EXPORT/ADR-054-<slug>.md`, extracted AFTER
