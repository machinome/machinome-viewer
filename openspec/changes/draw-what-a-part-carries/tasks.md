...
- [ ] 1.1 Export the framework's marked fixture from the solid-node checkout —
...
- [ ] 1.2 Write `tests/fixtures/marked/README.md` in the shape the `lock`,
...
- [ ] 1.3 Add `export_marked(target)` and `published_marked(target)` to
...
- [ ] 2.1 Red: extend `document.test.ts` with the refusals of design D6 — a
...
- [ ] 2.2 Add `ManifestMarking` to `types.ts` and `markings?: ManifestMarking[]`
...
- [ ] 3.1 Red: `tree.test.ts` — constructing a `WidgetTree` over a node
...
- [ ] 3.2 Red: the material carries `side: THREE.DoubleSide`,
  ...
  the roll.
- [ ] 3.3 Implement in `tree.ts`: the lazily created marking group (design D2),
  a `markings` record array in document order, and `loadMarking` reusing
  `loadMesh` with the marking material. Green.
- [ ] 3.4 Red then green: `applyVisibility` sets the marking group's own
...
- [ ] 4.1 Red then green: `tree.test.ts` — `assembly()` over the marked fixture
  returns exactly the snapshot the stripped twin returns: same nodes, paths,
  colours and `model` flags, and no node per marking.
- [ ] 4.2 Red then green: `navtree.test.ts` — `navigatorRows` over the marked
...
- [ ] 4.3 Red then green: hiding a marked part hides its decals and showing it
  restores them, driven through the same `setVisible` a host calls.

## 5. Reload on its own currency

- [ ] 5.1 Red: `tree.test.ts` reconcile cases — a marking whose `mtime` moved is
...
- [ ] 5.2 Red: `artifactChanged(decalPath)` refetches that marking only, and the
...
- [ ] 5.3 Red: a decal fetch that rejects leaves the whole previously rendered
  scene intact and a later update still succeeds (`viewer-package`, "A failed
  update leaves the model standing").
- [ ] 5.4 Implement the reconcile and `artifactChanged` routing in `tree.ts`
  (design D5: match by `name`, order by the document, fetch everything before
  the live tree is touched). Green.
- [ ] 5.5 Confirm `viewer.ts`'s `artifactChanged` handler needs no change beyond
...
- [ ] 7.1 Build the bundle in THIS worktree —
...
- [ ] 7.2 Red then green: a Playwright acceptance in `tests/test_widget_e2e.py`
...
- [ ] 7.3 Red then green: a capture test in `tests/test_capture.py` over
...
- [ ] 7.4 Inspect the pixels. If any part punches through its own decal, record
...
- [ ] 8.1 `cd solid_node_viewer/widget && npm run typecheck && npm test` — the
  whole widget suite, including the running corpus and the parity fixture,
  which this cycle must not move.
- [ ] 8.2 `/home/asa/devel/libresolid-studio/.venv/bin/python -m pytest` from the
...
- [ ] 8.3 README: a "What a part carries" section under the reading/driving
...
- [ ] 8.4 `CHANGELOG.md`: an entry under `0.2.0 — unreleased` in the voice of
...
- [ ] 8.5 Write the ADR **after** implementation, in `docs/adrs/EXPORT/` (the
