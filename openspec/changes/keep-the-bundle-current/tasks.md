...
- [ ] 1.1 `openspec validate keep-the-bundle-current --strict` green.
- [ ] 1.2 ADR-059 drafted — the viewer keeps its own bundle current, builds
...
- [ ] 2.1 `tests/test_currency.py`: a checkout whose newest widget source is
...
- [ ] 2.2 The reproduction of 0.3 as a test: with a bundle built from an
...
- [ ] 2.3 A stale bundle that cannot be rebuilt: dependencies absent →
...
- [ ] 2.4 The never-install guarantee: a rebuild driven with a recording
...
- [ ] 2.5 Concurrency: two threads finding the same bundle stale produce
...
- [ ] 2.6 An installed distribution: a tree with the bundle and
...
- [ ] 2.7 `tests/test_packaging.py`: a checkout whose bundle is STALE gets
      it rebuilt before a wheel is assembled. Red because
      `build_missing_frontends` builds only what is absent.
- [ ] 2.8 `tests/test_server.py`: a bundle request after a source changes
...
- [ ] 2.9 `tests/test_capture.py`: a capture whose bundle is stale copies
      the rebuilt bundle into staging, and one that cannot be rebuilt
      fails writing no image. Red today.
- [ ] 2.10 stdout hygiene: `describe` that rebuilds prints exactly one JSON
...
- [ ] 3.1 `solid_node_viewer/currency.py`, standard library only, importable
...
- [ ] 3.2 `ensure_current()`: return immediately when not a source checkout
...
- [ ] 3.3 The build invocation is `npm run build` in the widget directory
...
- [ ] 5.3 Confirm by diff that no file under `solid_node_viewer/widget/src/`,
      no number in `package.json`, and no ADR other than the new 059 changed.
- [ ] 5.4 End-to-end, in this worktree: reproduce 0.3 again, then ask
      `describe` and record that it now answers `apiVersion: 16` with a
      bundle at its `path` that carries `viewer API 16` and `[1..7]`.
- [ ] 5.5 `scripts/check-dist` is NOT run: it runs `npm ci` in the widget,
...
- [ ] 6.1 `CHANGELOG.md` under `0.2.0 — unreleased`: the bundle is kept
...
- [ ] 6.2 `README.md` "Working on the viewer": the manual
      ...
      (dependency installs).
- [ ] 6.3 `evidence.md`: §0's measurements with their after numbers, each
...
- [ ] 6.4 `openspec validate --strict` green; archive left for the reviewer,
