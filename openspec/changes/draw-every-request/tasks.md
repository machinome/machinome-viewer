...
- [ ] 1.1 `src/clockedControls.test.ts`, red: the module exports the gesture
      duration a drawn handle gesture takes, it is `0.2` seconds, and it does
      not depend on the nudge amount. Record the failure.
- [ ] 1.2 Add the constant beside `DEFAULT_NUDGE` with its reason (design §D2,
...
- [ ] 2.1 The RED for this wiring is a BROWSER one, honestly: `viewer.ts` has
      ...
      own suite.
- [ ] 2.2 Route the panel's `move` and `nudge` actions
...
- [ ] 2.3 `clockedDrawing` (`:421`) carries an optional indicator key instead
      of an instruction name; a handle's gesture passes none and indicates by
      following. No new chrome surface.
- [ ] 2.4 The CLOCK's own requests stay undrawn: `clockFrame` and the panel's
...
- [ ] 2.5 The host handle's `move` is unchanged and lands at once; a test
...
- [ ] 4.1 `tests/test_calculator_document.py`, red first, on the committed
...
- [ ] 4.2 Same file: a typed value committed on `crank`'s number field (typed,
      ...
      clause).
- [ ] 4.3 Same file: a gesture on the ranged input `operand` (`range: [0, 9]`,
...
- [ ] 4.4 Update the existing section 4 ("a gesture lands a running drawing"):
      ...
      drawing.
- [ ] 4.5 One screenshot mid-gesture kept beside ADR-064's two
      (`tests/_shots/`), showing the calculator part-way through a nudged
      stroke.

## 5. The Curta, measured where the finding came from

- [ ] 5.1 `tests/test_curta_drawing.py` (skippable, a measurement and not a
...
- [ ] 5.2 Record what the numbers say about the pilot's own question — whether
...
- [ ] 6.1 `docs/adrs/EXPORT/ADR-065`, written after implementation from what
...
- [ ] 6.2 `CHANGELOG.md` under `0.2.0 — unreleased`.
- [ ] 6.3 `openspec validate draw-every-request --strict`; sync the delta into
      `openspec/specs/viewer-package/spec.md` and archive the change.
- [ ] 6.4 Closing suites recorded: `npx tsc --noEmit`, `npx vitest run` (from
