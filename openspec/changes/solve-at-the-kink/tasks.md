...
- [ ] 1.1 RED FIRST, and it is the contract test: a test that classifies
      ...
      and the fixtures'.
- [ ] 1.2 The three-valued classification in `src/expressions.ts`, over
...
- [ ] 1.3 Unit tests for the table: `min(max((x-a)/b, 0), 1)` kinked;
...
- [ ] 1.4 Compute it at LOAD in `src/run/program.ts` — per plan skeleton,
...
- [ ] 2.1 The kink inventory in `src/expressions.ts`: the kink nodes of a
...
- [ ] 2.2 The breakpoint solve, mirroring `_KinkCuts.between`
...
- [ ] 3.1 RED (design §8 case B): a fixture whose gate's level reads a
      ...
      here.
- [ ] 3.2 Refactor the affine body of `crossingsOf` into a local
...
- [ ] 3.3 The kinked branch: breakpoints under the same `values` the
...
- [ ] 3.4 The boundary test: a surface lying EXACTLY on an interior
      breakpoint is located once, not twice and not zero times.

## 4. The self-read walk (design D4 (b))

- [ ] 4.1 RED (design §8 case C): the committed `clearing` fixture's cost
      as subexpression resolutions per tick, asserted at 32 004 today and
      under 4 000 after.
- [ ] 4.2 `Walk.crossing`, mirroring `_crossing`
...
- [ ] 4.3 The walk's own cuts gain the skeleton's kinks ONLY when the
...
- [ ] 4.4 Assert the cuts list stays sorted and that a breakpoint never
      reaches `crossings`, `land` or the crossing maximum (design D3).

## 5. The stop (design D4 (c))

- [ ] 5.1 RED (design §8 case A), as an IDENTITY and not within the
...
- [ ] 5.3 `edgeCuts` for a law with NO plan: its own kinks over the step
...
- [ ] 5.4 `planCuts` unions the skeleton's kinks into the partition,
      ...
      `pinion.turn`).
- [ ] 5.5 Assert a stop on a BLOCK coordinate is STILL searched:
      `RangedBlock`'s corpus entry and its cost number do not move.

## 6. Nothing else moves (design §8 case D, E)

- [ ] 6.1 The whole refreshed corpus replays, and the identity is
...
- [ ] 6.2 `Clearing`, `Train` and the Curta carriage cost EXACTLY 471.1,
      31.0 and 5 913.2 evaluations/tick, before and after.
- [ ] 6.3 Case E: a curved quantity is still searched — `max(0, sin(x))`
      ...
      starting point.
- [ ] 6.4 `src/running-corpus.json` is NOT touched by this cycle: confirm
      by diff.
- [ ] 6.5 `run/cost.test.ts`: raise the `clearing` fixture's floor to
...
- [ ] 7.1 `npx vitest run` and `npx tsc --noEmit` against task 0.1's
      numbers, with the new totals recorded. No `npm ci`, no
      `npm install`, no `scripts/check-dist`.
- [ ] 7.2 ADR-061 in `docs/adrs/EXPORT`, `a-kink-is-a-cut-in-the-viewer-too`
      ...
      chronological order.
- [ ] 7.3 `CHANGELOG.md` under `0.2.0 — unreleased`: ONE bullet — a
...
- [ ] 7.4 `evidence.md`: the measurements of §0 with the after numbers
...
- [ ] 7.5 `openspec validate solve-at-the-kink --strict` green, and the
