# Changelog entries before the first release

The viewer's first published version is 0.7.0, released on 21 September 2026
with Machinome 0.7.0 and numbered with it. The two development sections below,
written while the package was unreleased as 0.1.0 and then 0.2.0, are kept
here verbatim as the record of how each capability arrived, with the OpenSpec
change and decision each cites. `CHANGELOG.md` summarises the same work for a
reader of the released package. Nothing here was published under 0.1.0 or
0.2.0; the numbers name development states, not releases.
